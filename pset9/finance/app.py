...
def buy():
    ...
    if request.method == "POST":
        ...
        # Update history
        db.execute(
            "INSERT INTO transactions (user_id, symbol, shares, prices) VALUES(:user_id, :symbol, :shares, :price)",
            user_id  = session["user_id"],
            symbol = symbol,
            shares = shares,
            price = price
...
def register():
    ...
    if request.method == "POST":
        # Check requirements for create a credential
        if not request.form.get("username"):
        ...
        # Check if credential existe
        rows = db.execute("SELECT * FROM users WHERE username = ?", request.form.get("username"))
        if len(rows) != 0:
            return apology("Username alreaty exists", 400)
